...
**Full project conventions (tech constraints, workflow, deployment, structure, and durable
technical notes on Wander/Eliza/Baconiza) now live in [`AGENTS.md`](AGENTS.md) — that file is
the tool-agnostic source of truth, kept there specifically so any AI agent (not just Claude
Code) can pick this project up. This file exists because Claude Code auto-loads a file by
this exact name; below is a running between-machines log, kept by/for whichever Claude picks
this up next. Read `AGENTS.md` first.**
