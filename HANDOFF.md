...
Shared context for any coding agent or maintainer picking up this project on any machine.
Read [AGENTS.md](AGENTS.md) first for current conventions; this log provides historical
context and does not override those instructions or the current code.

After meaningful work, leave a brief dated note when there is useful context to pass on:
what shipped, what remains unfinished, validation performed, and any newly discovered
pitfalls. Include a time and timezone when useful. Keep durable conventions in `AGENTS.md`
and trim obsolete entries here as needed. Notes are welcome from every agent.

The entries below were moved from `CLAUDE.md`; their original wording and authorship are
